...
int main_compression_v4(int argc, char *argv[]){
...
int main_compression(int argc, char *argv[]){
    int i = 0, j, k, l, existe = 0, retour = 1, p;
    char caractere, nb[11] = {0}, c[MAX_CHAR] = {0}, *argv2[4], **liste_depart, **liste_tmp, nom_fichier[MAX_CHAR] = {0}, nom_fichier_temporaire[MAX_CHAR] = {0}, ajout_nom[11] = {0};
    FILE *fichier, *tmp;
    DIR *repertoire;
    struct dirent *liste;
    if(argc == 4){
        main_compression_v4(argc, argv);
        return 1;
    }else if(argc > 4){
        if(access(argv[2], F_OK)==0){
            printf("%s existe déjà.\n", argv[2]);
        ...
        liste_tmp = (char**)malloc((argc-3) * sizeof(char*));
        if(liste_tmp == NULL){
            printf("Erreur allocation mémoire.");
            return 0;
        }
        ...
        liste_depart = (char**)malloc(i * sizeof(char*));
        if(liste_depart == NULL){
            printf("Erreur allocation mémoire.");
            return 0;
        }
        for(k=0;k<i;k++){
            liste_depart[k] = (char*)malloc(MAX_CHAR * sizeof(char));
            if(liste_depart[k] == NULL){
                printf("Erreur allocation mémoire.");
                return 0;
            }
            strcpy(liste_depart[k], "\0");
        }
        i = 0;
        repertoire = opendir("./");
        while((liste = readdir(repertoire))){
            if(liste->d_type == 8){
                strcpy(liste_depart[i], liste->d_name);
                i++;
            }
        }
        closedir(repertoire);
        ...
        argv2[1] = NULL;
        for(k=3;k<argc;k++){
            argv2[2] = (char*)malloc((strlen(argv[2])+11)*sizeof(char));
            strcpy(argv2[2], argv[2]);
            sprintf(c, "%d", i);
            strcat(argv2[2], c);
            argv2[3] = (char*)malloc((strlen(argv[k]+1))*sizeof(char));
            strcpy(argv2[3], argv[k]);
            retour = main_compression_v4(4, argv2);
            if(retour == 0){
                return retour;
            }
        }
        ...
        while((liste = readdir(repertoire))){
            if(liste->d_type == 8){
                k = 0;
                for(j=0;j<i;j++){
                    if(strcmp(liste->d_name, liste_depart[j]) == 0){
                        k = 1;
                        break;
                    }
                }
                if(k == 0){
                    strcpy(liste_tmp[l], liste->d_name);
                    l++;
                }
            }
        }
        ...
        strcpy(nom_fichier, argv[2]);
        if(existe == 1){
            strcpy(nom_fichier_temporaire, argv[2]);
            while(access(nom_fichier_temporaire, F_OK)==0 && existe < INT_MAX){
            strcpy(nom_fichier_temporaire, argv[2]);
                sprintf(ajout_nom, "%d", existe);
                strcat(nom_fichier_temporaire, ajout_nom);
            ...
            strcpy(nom_fichier, nom_fichier_temporaire);
        }
        /*liste_comp = (char**)malloc((l+3) * sizeof(char*));
        for(i=0;i<l+3;i++){
            liste_comp[i] = (char*)malloc(MAX_CHAR * sizeof(char));
        }
        liste_comp[0] = NULL;
        liste_comp[1] = NULL;
        strcpy(liste_comp[2], nom_fichier);
        for(i=3;i<l+3;i++){
            strcpy(liste_comp[i], liste_tmp[i-3]);
            }*/
        /*main_compression_fichier_multiple(l+3, liste_comp);*/
        fichier = fopen(nom_fichier, "w");printf("%s\n", nom_fichier);
        sprintf(nb, "%d", l);
        fputs(nb, fichier);
        fputc('\n', fichier);
        for(i=0;i<l;i++){
            fputs(liste_tmp[i], fichier);
            fputc('\n', fichier);
        }
        for(i=0;i<l;i++){
            p = 1;
